...
a = np.array([a1, a2]).reshape(2, -1).T
a1, a2 = a.T
mask = lambda x: np.ma.masked_array(x, (a1 + a2 > 1))
π = mask(a @ p.f + p.ε - (a @ p.g - p.η) * p.b)
df = pd.DataFrame({"a1": a1, "a2": a2, "profit": π})
ax.contourf(a1, x, y, alpha=0.2)
